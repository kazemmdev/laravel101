...
@section('content')
   ...
   <form method="POST" action={{ route('tasks.update', $task->id) }} class="p-4 bg-white rounded-xl transition border border-solid border-gray-200 shadow">
      ...
      <div class="flex space-x-2">
         @include("common.search", ['tags' => $task->tags])
         <input type="datetime-local" id="expired_at" name="expired_at" value="{{ old("expired_at") ?? $task->expired_at }}" placeholder="expired_at" class="p-2 outline-none bg-transparent mb-1" />
      </div>
